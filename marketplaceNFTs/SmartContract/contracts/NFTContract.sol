...
contract NFTContract is ERC721URIStorage {
    ...
    uint256 NftPrice = 0.0015 ether;

    address payable owner;

    mapping(uint256 => MarketItem) private idMarketItem;

    //les informations du NFT
...
// seulement l'Owner de NFT qui peut changer le prix de NFT sinon il s'affiche un message d'erreur
    modifier onlyOwner() {
        require(
            msg.sender == owner,
            "only owner of the marketplace can change the Nft price"
        );
        _;
    }
    ...
    function updateNftPrice(uint256 _NftPrice)
        public
        payable
    ...
    function getNftPrice() public view returns (uint256) { //cette fonction est public pour que everone can use this function
        return NftPrice;
    }
    ...
    function createMarketItem(uint256 tokenId, uint256 price) private {
        ...
        require(
            msg.value == NftPrice,
            "Pric must be equal to Nft price"
        );
